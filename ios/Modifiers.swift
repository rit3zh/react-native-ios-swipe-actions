    ...
    for mod in mods {
      for (key, value) in mod {
        ...
        case "datePickerStyle":
            if let datePickerStyle = value as? String{
                switch datePickerStyle {
                case "compact":
                    if #available(iOS 14.0, *) {
                        view = AnyView(view.datePickerStyle(.compact))
                    }
                case "graphical":
                    if #available(iOS 14.0, *) {
                        view = AnyView(view.datePickerStyle(.graphical))
                    }
                case "wheel":
                    if #available(iOS 14.0, *) {
                        view = AnyView(view.datePickerStyle(.wheel))
                    }
                    
                case "automatic":
                    if #available(iOS 14.0, *) {
                        view = AnyView(view.datePickerStyle(.automatic))
                    }
                default:
                    break
                }
